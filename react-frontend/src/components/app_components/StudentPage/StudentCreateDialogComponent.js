...
const StudentCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.nameyhhjd)) {
                error["nameyhhjd"] = `Nameyhhjd field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            nameyhhjd: _entity?.nameyhhjd,Image: _entity?.Image,
    ...
    return (
        <Dialog header="Create Student" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                <label htmlFor="nameyhhjd">Nameyhhjd:</label>
                <InputText id="nameyhhjd" className="w-full mb-3 p-inputtext-sm" value={_entity?.nameyhhjd} onChange={(e) => setValByKey("nameyhhjd", e.target.value)}  required  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["nameyhhjd"]) ? (
              <p className="m-0" key="error-nameyhhjd">
                {error["nameyhhjd"]}
              </p>
            ) : null}
          </small>
...
<div className="col-12 field">
                    <span className="align-items-center">
                        ...
                        <UploadFilesToS3 type={'create'} user={props.user} id={urlParams.id} serviceName="student" onUploadComplete={setImageId} onFileLoaded={onFileImageLoaded}/>
                    </span>
                    <small className="p-error">
